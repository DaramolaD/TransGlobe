...
export default function ResourcesPage() {
  ...
  return (
    <div className="min-h-screen bg-background scroll-smooth">
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            {industryReports.map((report, index) => (
              <motion.div
                ...
                className="bg-platinum-off rounded-2xl p-8 border border-border/50 hover:shadow-lg hover:-translate-y-1 transition-all duration-300"
              >
                <div className="flex items-start justify-between mb-6">
                  <div className="w-16 h-16 icon-tile rounded-xl flex items-center justify-center">
                    <BarChart3 className="w-8 h-8 text-white" />
                  </div>
                  <div className="text-right">
                    <span className="inline-block px-3 py-1 bg-ember-main/10 text-graphite-mid text-sm font-medium rounded-full">
                      {report.category}
                    </span>
                ...
                <div className="mb-6">
                  <h4 className="font-semibold mb-3 text-graphite-dark">Key Insights</h4>
                  <ul className="space-y-2">
                    {report.keyInsights.slice(0, 3).map((insight, idx) => (
                      <li key={idx} className="text-sm text-muted-foreground flex items-start space-x-2">
                        <CheckCircle className="w-4 h-4 text-sage-main flex-shrink-0 mt-0.5" />
                        <span>{insight}</span>
                      </li>
                    ))}
                  </ul>
      ...
      {/* Case Studies */}
      <section id="case-studies" className="py-20 bg-platinum-off">
        <div className="container mx-auto px-4">
          <motion.div
            initial={{ opacity: 0, y: 40 }}
            whileInView={{ opacity: 1, y: 0 }}
              ...
              >
                  ...
                  <ul className="space-y-2">
                    {study.results.map((result, idx) => (
                      <li key={idx} className="text-sm text-muted-foreground flex items-center space-x-2">
                        <TrendingUp className="w-4 h-4 text-sage-main flex-shrink-0" />
                        <span>{result}</span>
                      </li>
                    ))}
                  </ul>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-border/30">
                  ...
                  <Link href={`/resources/case-studies/${study.id}`}>
                    <motion.button
                      className="px-6 py-2 text-graphite-mid hover:text-ember-main font-medium flex items-center space-x-2 transition-colors"
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {bestPractices.map((practice, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ duration: 0.6, delay: index * 0.1 }}
                className="bg-platinum-off rounded-2xl p-6 border border-border/50 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group"
              >
                ...
                <div className="mb-4">
                  <span className="inline-block px-2 py-1 bg-ember-main/10 text-graphite-mid text-xs font-medium rounded-full mb-2">
                    {practice.category}
                  </span>
                  ...
                  <ul className="space-y-1">
                    {practice.keyPoints.slice(0, 2).map((point, idx) => (
                      <li key={idx} className="text-xs text-muted-foreground flex items-center space-x-1">
                        <CheckCircle className="w-3 h-3 text-sage-main flex-shrink-0" />
                        <span>{point}</span>
                      </li>
                    ))}
                  </ul>
      ...
      {/* Global Insights */}
      <section id="global-insights" className="py-20 bg-platinum-off">
        <div className="container mx-auto px-4">
          <motion.div
            initial={{ opacity: 0, y: 40 }}
            whileInView={{ opacity: 1, y: 0 }}
                  ...
                  <div>
                    ...
                    <ul className="space-y-2">
                      {insight.opportunities.map((opportunity, idx) => (
                        <li key={idx} className="text-sm text-muted-foreground flex items-start space-x-2">
                          <TrendingUp className="w-4 h-4 text-graphite-mid flex-shrink-0 mt-0.5" />
